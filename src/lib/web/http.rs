...
pub async fn get_clip(
    ...
    fn render_with_status<T: ctx::PageContext + serde::Serialize + std::fmt::Debug>(
        ...
        let html = Html(renderer.render(context, &[]));
        Ok(status::Custom(status, html))
    }
...
pub async fn get_raw_clip(
    ...
    match action::get_clip(req, data.get_pool()).await {
        Ok(clip) => {
            hit_counter.hit(shortcode.clone(), 1);
...
pub mod catcher {
    ...
    #[catch(default)]
    fn default(request: &Request) -> &'static str {
        eprintln!("{}", request.uri());
        eprintln!("General error: {:?}", request);
